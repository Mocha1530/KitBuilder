...
class KitManager {
  constructor() {
    ...
    kitNameInput.addEventListener('input', () => {
      setTimeout(() => {
        output.value = this.generateCommands();
      }, 0);
    });
    const output = document.getElementById("kit-output");

    // Add event listene  r to update commands when kit items change
  ...
  generateCommands() {
    const kitName = document.getElementById('kit-name').value.trim() || "MyKit";
  ...
  kitData.items.forEach(item => {
    ...
    if (slot) {
      ...
      slotItem.addEventListener('dragstart', (ev) => {
        const qtyInput = slot.querySelector('.qty-input');
        const currentQuantity = qtyInput ? parseInt(qtyInput.value) : 1;
  ...
  document.querySelectorAll('.slot').forEach(slot => {
    ...
    slot.addEventListener('drop', (e) => {
      ...
      if (data.slotType === slot.dataset.slotType) {
        ...
        newSlotImg.src = this.buildImageUrl(this.findItemImagePath(data.itemId));
        newSlotImg.onerror = () => {
          newSlotImg.src = "https://kb.veretech.systems/images/placeholder.png";
        };
        ...
        newQtyInput.value = data.quantity || 1;
        newQtyInput.min = 1;
        ...
        newSlotItem.addEventListener('dragstart', (ev) => {
          const currentQtyInput = newSlotItem.querySelector('.qty-input');
          const currentQuantity = currentQtyInput ? parseInt(currentQtyInput.value) : 1;
          ev.dataTransfer.setData('text/plain', JSON.stringify({
            itemId: data.itemId,
...
parseAndImportKit(importText) {
  ...
  items.forEach(item => {
    let slotType = 'main'; // Default to main
    if (item.container.toLowerCase() === 'wear') slotType = 'wear';
    else if (item.container.toLowerCase() === 'belt') slotType = 'belt';
